...
import { graphCall } from './whatsapp.js';
...
export async function getProducts(env: Env): Promise<Product[]> {
  ...
  const cached = await getStore(env).get(SHOPIFY_CACHE_KEY);
  ...
  await getStore(env).put(SHOPIFY_CACHE_KEY, JSON.stringify(mapped), { expirationTtl: 600 });
  return mapped;
}
...
export async function shopifyAccessToken(env: Env, force = false): Promise<string | null> {
  ...
  if (!force) {
    const cached = await getStore(env).get(SHOPIFY_TOKEN_KEY);
  ...
  await getStore(env).put(SHOPIFY_TOKEN_KEY, body.access_token, { expirationTtl: ttl });
