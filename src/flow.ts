...
import { COPY, cap, fill, formatINR } from './copy.js';
import {
  CATEGORIES,
  OCCASIONS,
  categoryLabel,
...
import { rankLooks } from './ranking.js';
import {
  sendButtons,
...
export async function loadState(env: Env, waId: string): Promise<State> {
  const raw = await getStore(env).get(`state:${waId}`);
...
export async function saveState(env: Env, waId: string, state: State): Promise<void> {
  ...
  await getStore(env).put(`state:${waId}`, JSON.stringify(state), {
    expirationTtl: STATE_TTL_SECONDS,
  });
}

export async function clearState(env: Env, waId: string): Promise<void> {
  await getStore(env).delete(`state:${waId}`);
}
...
export async function sendLooks(
   ...
   *
   * Names are captured alongside: the restock list is read by people who buy
   * stock, and the products that matter most there are the ones that never
   * sold — so a title cannot be recovered from order data later.
   */
  if (products.length) {
...
export async function runBackend(env: Env, to: string, state: State): Promise<void> {
  ...
  if (intro) {
    const analytics = getAnalytics(env);
    await analytics.milestone('widened', {
      waId: to,
      sessionId: state.sessionId,
      flowStep: state.step,
      meta: { occasion: state.occasion, category: state.category },
    });
    if (state.sessionId) await analytics.patchSession(state.sessionId, { widened: true });
  }
...
export async function askSize(env: Env, to: string, state: State, product: Product): Promise<void> {
  ...
  if (sizes.length === 0) {
    ...
    await analytics.milestone('size_sold_out', {
      waId: to,
      sessionId: state.sessionId,
      flowStep: 'size',
...
export async function sendCheckout(
  ...
  /*
   * The last step the bot can observe. Everything past this happens on the
   * hosted checkout, which a Worker cannot see — so the order itself is
   * learned later from Shopify, matched on the UTMs checkoutUrl() stamps here.
   */
  const analytics = getAnalytics(env);
  if (state.sessionId) {
    await analytics.patchSession(state.sessionId, {
  ...
  await analytics.milestone('checkout_opened', {
    waId: to,
    sessionId: state.sessionId,
    flowStep: 'checkout',
    productIds: [product.id],
